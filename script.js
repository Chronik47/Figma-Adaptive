const productCards = JSON.parse(product);
const card = document.querySelector(".catalog");
const conteiner = document.querySelector(".change-list");
...
productCards.reverse().forEach((element) => {
  ...
  <div class="fon_img">
    <img
      src="${element.img}"
      alt="man in a jacket"
      class="catalog_card_img"
    />
    <button class="catalog_card_btn">
  ...
  <div class="catalog_card_content">
    <h4 class="catalog_card_titl">${element.title}</h4>
    <p class="catalog_card_text">${element.text}</p>
    <span class="catalog_card_prise">${element.prise}</span>
  </div>
</article>`
  );

  const cardButton = document.querySelector(".catalog_card_btn");
  let count = 0;
  cardButton.addEventListener("click", () => {
    if (conteiner.children.length === 0) {
      conteiner.insertAdjacentHTML(
        "beforebegin",
        `
        <div class="change-list__block">
      <h1 class="change-list__title">Cart items</h1>
      </div>
      `
      );
      conteiner.classList.add("change-list__padding");
    }
    if (count === 0) {
      conteiner.insertAdjacentHTML(
        "afterbegin",
      ...
      <div class="change-list__item__content">
        ...
        <h2 class="change-list__item_content__title">
        ${element.heading}
        </h2>
        <p class="change-list__item_content__price">
          Price: <span>${element.prise}</span>
        </p>
        <p class="change-list__item_content__color">
          Color: <span>${element.color}</span>
        </p>
        <p class="change-list__item_content__size">Size: <span>${element.size}</span></p>
        <p class="change-list__item_content__quantity">
        Quantity:
          <input
            class="change-list__item_content__input"
            type="number"
...
`
      ...
      const card = document.querySelector(".change-list__item");
      const title = document.querySelector(".change-list__title");
      close.addEventListener("click", () => {
        card.remove();
        if (conteiner.children.length === 0) {
          title.remove();
          conteiner.classList.remove("change-list__padding");
        }
        count = 0;
      });
    }
